...
## Azure App Service 部署說明

本專案已將前端 Vue 與後端 Express 放在同一個專案根目錄。GitHub Actions 設定檔位於：

```text
.github/workflows/azure-webapp.yml
```

部署前請到 GitHub Repository Settings → Secrets and variables → Actions 新增：

```text
AZURE_WEBAPP_PUBLISH_PROFILE
```

內容貼上 Azure App Service 下載的 Publish profile。接著打開 `.github/workflows/azure-webapp.yml`，把：

```yaml
AZURE_WEBAPP_NAME: fcu-parking-system
```

改成自己的 Azure App Service 名稱。
...
Express 會提供 `dist` 前端檔案，API 則使用 `/api` 路徑。
